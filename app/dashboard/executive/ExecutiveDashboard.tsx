...
export default function ExecutiveDashboard() {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        if (visitorsResponse.status === 'fulfilled' && visitorsResponse.value.ok) {
          ...
          // Filter visitors for this executive's region/department if applicable
          if (user?.role !== 'admin' && user?.region) {
            visitors = visitors.filter((v: any) => !v.region || v.region === user.region);
          ...
          console.log('✅ Executive visitors data loaded:', totalVisitors, 'total visitors');
        } else {
          console.warn('⚠️ Failed to load visitors data, using fallback');
          totalVisitors = 25; // Fallback for executives (less than admin)
          leadsConverted = 4;
        }
        ...
        const todayMessages = Math.floor(todayVisitors * 0.6);

        setTotals({
          visitors: totalVisitors,
          messages: Math.floor(totalVisitors * 0.3),
          faqs: 6, // Less than admin
          articles: 8 // Less than admin
        });
        setToday({
        ...
        if (dailyAnalysisResponse.status === 'fulfilled' && dailyAnalysisResponse.value.ok) {
          ...
          console.log('📊 Executive Daily Analysis Data:', dailyAnalysis);
        ...
        if (recentConversationsResponse.status === 'fulfilled' && recentConversationsResponse.value.ok) {
          ...
          console.log('📊 Executive Recent Conversations:', recentConversations);
        ...
        setTotals({ visitors: 25, messages: 8, faqs: 6, articles: 8 });
        setToday({ visitors: 3, messages: 2 });
        setDailyVisitorsData({
          labels: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'],
          datasets: [{
            label: 'Daily Visitors',
            data: [8, 12, 6, 10, 15, 11, 18],
            borderColor: 'rgb(59, 130, 246)',
            backgroundColor: 'rgba(59, 130, 246, 0.1)',
            tension: 0.4
          }]
        });
        setConversationRatioData({ leadsConverted: 4, visitors: 25, conversionRate: 16 });
        setDailyAnalysisData([]);
        setRecentConversationsData([]);
      } finally {
        setLoading(false);
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      <Sidebar userRole={user?.role || "executive"} userName={user?.name || "Executive"} />

      <div className="flex-1 flex flex-col overflow-hidden">
        <DashboardHeader userRole={user?.role || "executive"} userName={user?.name || "Executive"} />
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-600">
                  Welcome back, {user?.name || "Executive"}! Here&apos;s your performance overview.
                </p>
              </div>
            </div>
          </div>
